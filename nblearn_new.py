...
def init_cond_all():
    global cond_all
    for word in cnt_all_words:
        cond_all[word] = {}
        cond_all[word]['truthful'] = 0
        cond_all[word]['deceptive'] = 0
        cond_all[word]['positive'] = 0
        cond_all[word]['negative'] = 0
...
def write_final_model():
    global cond_all
    global priors
    final_model['priors'] = priors
    for word in cond_all:
        cond_all_prob[word] = {}
        # sum(a_dict.values(), 0.0))
        alpha = len(cnt_all_words.keys())
        if word in cnt_trust_true:
            cond_all_prob[word]['truthful'] = (cond_all[word]['truthful'] + 1) / (alpha + (sum(cnt_trust_true.values()) * 1.0))
        else:
            cond_all_prob[word]['truthful'] = 1.0 / (alpha + (sum(cnt_trust_true.values()) * 1.0))

        if word in cnt_trust_false:
            cond_all_prob[word]['deceptive'] = (cond_all[word]['deceptive'] + 1) / (alpha + (sum(cnt_trust_false.values()) * 1.0))
        else:
            cond_all_prob[word]['deceptive'] = 1.0 / (alpha + (sum(cnt_trust_false.values()) * 1.0))

        if word in cnt_senti_true:
            cond_all_prob[word]['positive'] = (cond_all[word]['positive'] + 1) / (alpha + (sum(cnt_senti_true.values()) * 1.0))
        else:
            cond_all_prob[word]['positive'] = 1.0 / (alpha + (sum(cnt_senti_true.values()) * 1.0))

        if word in cnt_senti_false:
            cond_all_prob[word]['negative'] = (cond_all[word]['negative'] + 1) / (alpha + (sum(cnt_senti_false.values()) * 1.0))
        else:
...
def main():
    ...
    priors['truthful'] = p_true
    priors['deceptive'] = p_deceptive
